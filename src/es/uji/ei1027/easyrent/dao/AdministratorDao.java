...
public class AdministratorDao {
	...
	public void addAdministrator(Administrator administrator) throws PSQLException{
		...
		this.jdbcTemplate.update("INSERT INTO Administrator(username, national_id, name, surname, email, postal_address, registration_date, phone_number) VALUES(?, ?, ?, ?, ?, ?, ?, ?);", administrator.getUsername(), administrator.getNationalId(), administrator.getName(), administrator.getSurname(), administrator.getEmail(), administrator.getPostalAddress(), new java.sql.Date(Integer.parseInt(fecha[0])-1900,Integer.parseInt(fecha[1])-1,Integer.parseInt(fecha[2])), administrator.getPhoneNumber());
	}
		
	public void updateAdministrator(Administrator administrator) {
		String []fecha = administrator.getRegistrationDate().split("-");
		this.jdbcTemplate.update("UPDATE Administrator SET national_id = ?, name = ?, surname = ?, email = ?, postal_address = ?, registration_date = ?, phone_number = ? WHERE username = ?;", administrator.getNationalId(), administrator.getName(), administrator.getSurname(), administrator.getEmail(), administrator.getPostalAddress(), new java.sql.Date(Integer.parseInt(fecha[0])-1900,Integer.parseInt(fecha[1])-1,Integer.parseInt(fecha[2])), administrator.getPhoneNumber(), administrator.getUsername());
	}

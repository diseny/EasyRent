...
class CheckinValidator implements Validator { 
	...
	public void validate(Object obj, Errors errors) {
		...
		if(user.getPassword().trim().equals(""))
			errors.rejectValue("password", "required", "La contrase√±a es un campo obligatorio.");
	}
   ...
   public void setUserDao(UserDao userDao) {
	...
	public String checkLogin(@ModelAttribute("user") User user, BindingResult bindingResult, HttpSession session) {
		...
		try{
			...
			bindingResult.rejectValue("username", "registered", "El username ya est· registrado, por favor pruebe con otro.");
			return "user/checkin";
		}
